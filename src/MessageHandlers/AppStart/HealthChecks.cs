...
{
    public static IServiceCollection AddFunctionHealthChecks(this IServiceCollection services, ApplicationSettings applicationSettings, bool requiresAzureAuthentication)
    {
        services.AddSingleton(sp => new FunctionHealthChecker(
            new DbHealthCheck(applicationSettings.DbConnectionString.EnsureAzureAdAuthentication(requiresAzureAuthentication), sp.GetService<ILogger<DbHealthCheck>>()!),
